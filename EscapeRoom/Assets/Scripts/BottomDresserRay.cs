...
public class BottomDresserRay : MonoBehaviour
{
    ...
    private BottomDresserController raycasted_obj;
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, fwd, out hit, rayLength, mask))
        {
            if (hit.collider.CompareTag(interactableTag))
            {
                if (!doOnce)
                {
                    raycasted_obj = hit.collider.gameObject.GetComponent<BottomDresserController>();
