...
public class LeftDoorRaycast : MonoBehaviour
{
    ...
    private LeftDoorController raycasted_obj;
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, fwd, out hit, rayLength, mask))
        {
            if (hit.collider.CompareTag(interactableTag))
            {
                if (!doOnce)
                {
                    raycasted_obj = hit.collider.gameObject.GetComponent<LeftDoorController>();
